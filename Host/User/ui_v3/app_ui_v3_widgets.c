...
void ui3_topbar(ui3_layout_t *lo, ui3_state_t *st)
{
    ...
    st->cloud_badge = lo->cloud;
    st->cloud_dot = lv_obj_create(lo->cloud);
    lv_obj_remove_style_all(st->cloud_dot);
    lv_obj_set_size(st->cloud_dot, 8, 8);
    lv_obj_set_style_radius(st->cloud_dot, LV_RADIUS_CIRCLE, 0);
    lv_obj_set_style_bg_color(st->cloud_dot, st->mqtt_online ? UI3_COL_SAGE_MID : UI3_COL_INK_SOFT, 0);
    lv_obj_set_style_bg_opa(st->cloud_dot, LV_OPA_COVER, 0);

    st->cloud_label = lv_label_create(lo->cloud);
    lv_label_set_text(st->cloud_label, st->mqtt_online ? "云端" : "离线");
    lv_obj_set_style_text_font(st->cloud_label, UI3_FONT_TITLE, 0);
    lv_obj_set_style_text_color(st->cloud_label, st->mqtt_online ? UI3_COL_SAGE_MID : UI3_COL_INK_SOFT, 0);
    lv_obj_set_style_pad_ver(st->cloud_label, 0, 0);
}

void ui3_topbar_cloud_refresh(ui3_state_t *st)
{
    if(st == NULL || st->cloud_badge == NULL || !lv_obj_is_valid(st->cloud_badge)) {
        return;
    }
    lv_obj_set_style_bg_color(st->cloud_badge,
                              st->mqtt_online ? UI3_COL_CLOUD_BG : UI3_COL_PAPER_DEEP, 0);
    if(st->cloud_dot != NULL && lv_obj_is_valid(st->cloud_dot)) {
        lv_obj_set_style_bg_color(st->cloud_dot,
                                  st->mqtt_online ? UI3_COL_SAGE_MID : UI3_COL_INK_SOFT, 0);
    }
    if(st->cloud_label != NULL && lv_obj_is_valid(st->cloud_label)) {
        lv_label_set_text(st->cloud_label, st->mqtt_online ? "云端" : "离线");
        lv_obj_set_style_text_color(st->cloud_label,
                                    st->mqtt_online ? UI3_COL_SAGE_MID : UI3_COL_INK_SOFT, 0);
    }
}
...
void ui3_page_head_menu(ui3_layout_t *lo, const char *title)
{
    lv_obj_t *h = lv_label_create(lo->root);
    lv_label_set_text(h, title);
    lv_obj_set_style_text_font(h, UI3_FONT_MENU, 0);
    lv_obj_set_style_text_color(h, UI3_COL_INK, 0);
    lv_obj_align(h, LV_ALIGN_TOP_LEFT, 16, 32);
}
...
static void ui3_viewport_scroll_cb(lv_event_t *e)
{
    lv_obj_t *track = (lv_obj_t *)lv_event_get_user_data(e);
    ui3_gesture_sync_scroll_px(ui3_state(), track);
}
...
lv_obj_t *ui3_scroll_viewport(ui3_layout_t *lo, int16_t h)
{
    ...
    lv_obj_set_scroll_snap_y(vp, LV_SCROLL_SNAP_NONE);

    track = lv_obj_create(vp);
    ...
    lv_obj_set_style_pad_row(track, 8, 0);
    lv_obj_add_event_cb(vp, ui3_viewport_scroll_cb, LV_EVENT_SCROLL, track);
    lo->scroll_track = track;
    return track;
}
...
typedef struct {
    lv_event_cb_t on_ok;
    lv_event_cb_t on_esc;
    void *user;
} ui3_footer_cb_t;

static ui3_footer_cb_t s_footer_cb;

void ui3_footer_sel_refresh(void)
{
    ...
    if(st->footer_btn_back != NULL && lv_obj_is_valid(st->footer_btn_back)) {
        ui3_dock_btn_set_sel(st->footer_btn_back, st->footer_back_fill != 0u,
                             st->footer_sel == UI3_FOOTER_SEL_BACK);
    }
    if(st->footer_btn_ok != NULL && lv_obj_is_valid(st->footer_btn_ok)) {
        ui3_dock_btn_set_sel(st->footer_btn_ok, st->footer_ok_fill != 0u,
                             st->footer_sel == UI3_FOOTER_SEL_OK);
    }
}

static void ui3_footer_fire(uint8_t which)
{
    lv_event_cb_t cb = (which == UI3_FOOTER_SEL_BACK) ? s_footer_cb.on_esc : s_footer_cb.on_ok;
...
void ui3_footer_on_key(uint8_t which)
{
    ...
    if(which == UI3_FOOTER_SEL_BACK && st->footer_btn_back == NULL) {
        return;
    }
    if(which == UI3_FOOTER_SEL_OK && st->footer_btn_ok == NULL) {
        return;
    }
    if(st->footer_sel != which) {
        st->footer_sel = which;
        ui3_footer_sel_refresh();
        return;
    }
    st->footer_sel = UI3_FOOTER_SEL_NONE;
    ui3_footer_sel_refresh();
    ui3_footer_fire(which);
}

static void cb_footer_ok(lv_event_t *e)
{
    (void)e;
    ui3_footer_on_key(UI3_FOOTER_SEL_OK);
}

static void cb_footer_esc(lv_event_t *e)
{
    (void)e;
                          ...
                          bool primary_fill)
{
    ...
    s_footer_cb.on_ok = on_ok;
    s_footer_cb.on_esc = on_esc;
    s_footer_cb.user = user;
    if(st != NULL) {
        st->footer_btn_back = NULL;
        st->footer_btn_ok = NULL;
        st->footer_ok_fill = primary_fill ? 1u : 0u;
        st->footer_back_fill = 0u;
        st->footer_sel = UI3_FOOTER_SEL_NONE;
    }
    ...
    if(show_esc) {
        ...
        if(st != NULL) {
            st->footer_btn_back = esc;
        }
        lv_obj_add_event_cb(esc, cb_footer_esc, LV_EVENT_CLICKED, user);
    }
    lv_obj_t *primary = ui3_dock_btn(lo->footer, ok, primary_fill, 0);
    lv_obj_set_flex_grow(primary, 1);
    if(st != NULL) {
        st->footer_btn_ok = primary;
    }
    lv_obj_add_event_cb(primary, cb_footer_ok, LV_EVENT_CLICKED, user);
    return lo->footer;
}
...
lv_obj_t *ui3_footer_pair(ui3_layout_t *lo, lv_event_cb_t on_esc, lv_event_cb_t on_regen, void *user)
{
    ...
    if(st != NULL) {
        st->footer_btn_ok = reg;
    }
    lv_obj_add_event_cb(reg, cb_footer_ok, LV_EVENT_CLICKED, user);
    return lo->footer;
}
...
static void field_box_sync_caret(lv_obj_t *box, lv_obj_t *t, bool focus)
{
    ...
    if(focus) {
        ...
        if(st != NULL) {
            st->active_caret = caret;
        }
    } else {
        if(caret != NULL && lv_obj_is_valid(caret)) {
            if(st != NULL && st->active_caret == caret) {
...
void ui3_caret_poll(void)
{
    ...
    last_ms = now;
    caret_on ^= 1u;
    if(st != NULL && st->active_caret != NULL && lv_obj_is_valid(st->active_caret)) {
        lv_obj_set_style_bg_opa(st->active_caret, caret_on ? LV_OPA_COVER : LV_OPA_TRANSP, 0);
    }
}
...
bool ui3_inputs_live_refresh(ui3_state_t *st)
{
    ...
    case UI3_SCR_EDIT_USER:
        if(st->inp_count < 1u || st->edit_pwd_editing == 0u) {
            return false;
        }
        ui3_pwd_mask_fill(mask, sizeof(mask), st->edit_pwd);
        ui3_field_wrap_update(st->inp_wrap[0], mask, st->edit_field == 1u, false);
        break;
    default:
        return false;
    }
                          ...
                          bool disabled, lv_event_cb_t on_tap, void *user)
{
    ...
    lv_obj_set_style_pad_bottom(wrap, 4, 0);
...
static void role_seg_style(lv_obj_t *btn, bool selected)
{
    ...
    if(selected) {
        lv_obj_set_style_bg_color(btn, UI3_COL_SURFACE, 0);
        lv_obj_set_style_bg_opa(btn, LV_OPA_COVER, 0);
        if(lbl) {
            lv_obj_set_style_text_color(lbl, UI3_COL_INK, 0);
        }
    } else {
        lv_obj_set_style_bg_opa(btn, LV_OPA_TRANSP, 0);
        if(lbl) {
            lv_obj_set_style_text_color(lbl, UI3_COL_INK_SOFT, 0);
        }
    }
}
                          ...
                          lv_event_cb_t on_user, lv_event_cb_t on_admin, void *user)
{
    ...
    lab = lv_label_create(wrap);
    lv_label_set_text(lab, "身份");
    lv_obj_add_style(lab, ui3_style_label(), 0);
    lv_obj_set_style_text_color(lab, UI3_COL_INK_SOFT, 0);

    track = lv_obj_create(wrap);
    lv_obj_remove_style_all(track);
    lv_obj_set_width(track, lv_pct(100));
    lv_obj_set_height(track, 38);
    lv_obj_set_style_bg_color(track, UI3_COL_PAPER_DEEP, 0);
    lv_obj_set_style_bg_opa(track, LV_OPA_COVER, 0);
    lv_obj_set_style_radius(track, 14, 0);
    lv_obj_set_style_border_width(track, 1, 0);
    lv_obj_set_style_border_color(track, UI3_COL_LINE, 0);
    lv_obj_set_style_pad_all(track, 4, 0);
    lv_obj_set_flex_flow(track, LV_FLEX_FLOW_ROW);
    lv_obj_set_flex_align(track, LV_FLEX_ALIGN_SPACE_BETWEEN, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
    lv_obj_clear_flag(track, LV_OBJ_FLAG_SCROLLABLE);
    if(focus) {
        lv_obj_set_style_border_color(track, UI3_COL_TERRA, 0);
        lv_obj_set_style_border_width(track, 2, 0);
    }

    u = lv_obj_create(track);
    lv_obj_remove_style_all(u);
    lv_obj_set_height(u, 30);
    lv_obj_set_flex_grow(u, 1);
    lv_obj_add_flag(u, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_clear_flag(u, LV_OBJ_FLAG_SCROLLABLE);
    if(on_user) {
        lv_obj_add_event_cb(u, on_user, LV_EVENT_CLICKED, user);
    }
    lv_obj_t *ul = lv_label_create(u);
    lv_label_set_text(ul, "用户");
    lv_obj_add_style(ul, ui3_style_label(), 0);
    lv_obj_center(ul);

    a = lv_obj_create(track);
    ...
    if(on_admin) {
        lv_obj_add_event_cb(a, on_admin, LV_EVENT_CLICKED, user);
    }
    lv_obj_t *al = lv_label_create(a);
    lv_label_set_text(al, "管理员");
    lv_obj_add_style(al, ui3_style_label(), 0);
    lv_obj_center(al);

    role_seg_style(u, !is_admin);
    role_seg_style(a, is_admin);
    return wrap;
}
...
void ui3_role_picker_set(lv_obj_t *wrap, bool is_admin, bool focus)
{
    ...
    track = lv_obj_get_child(wrap, 1);
    if(track == NULL) {
        return;
    }
    u = lv_obj_get_child(track, 0);
    a = lv_obj_get_child(track, 1);
    if(u && a) {
        role_seg_style(u, !is_admin);
        role_seg_style(a, is_admin);
        lv_obj_set_style_border_color(track, focus ? UI3_COL_TERRA : UI3_COL_LINE, 0);
        lv_obj_set_style_border_width(track, focus ? 2 : 1, 0);
    }
}
...
void ui3_card_set_sel(lv_obj_t *card, bool sel, bool menu_style)
{
    ...
    if(menu_style) {
        ...
        if(ico) {
            ...
            lv_obj_invalidate(ico);
        }
    }
}

#define UI3_MENU_ICON_X    4
#define UI3_MENU_ICON_SZ   30
#define UI3_MENU_TEXT_X    40

static void menu_icon_line(lv_draw_ctx_t *ctx, lv_color_t col, lv_coord_t x1, lv_coord_t y1,
                           lv_coord_t x2, lv_coord_t y2)
{
    ...
    pts[1].x = x2;
    pts[1].y = y2;
    lv_draw_line(ctx, &dsc, &pts[0], &pts[1]);
}

static void menu_icon_draw_cb(lv_event_t *e)
{
    lv_obj_t *obj;
    lv_draw_ctx_t *draw_ctx;
    lv_area_t a;
    ui3_menu_icon_t kind;
    lv_color_t col;
    lv_coord_t cx;
    lv_coord_t cy;
    ...
    kind = (ui3_menu_icon_t)(uintptr_t)lv_obj_get_user_data(obj);
    lv_obj_get_content_coords(obj, &a);
    col = lv_obj_get_style_border_color(obj, 0);
    if(col.full != UI3_COL_TERRA.full) {
        col = UI3_COL_INK_SOFT;
    }
    cx = (lv_coord_t)((a.x1 + a.x2) / 2);
    cy = (lv_coord_t)((a.y1 + a.y2) / 2);

    switch(kind) {
    case UI3_MENU_ICON_USER:
        lv_draw_rect_dsc_init(&rd);
        rd.bg_opa = LV_OPA_TRANSP;
        rd.border_color = col;
        rd.border_width = 2;
        rd.radius = LV_RADIUS_CIRCLE;
        a.x1 = (lv_coord_t)(cx - 4);
        a.y1 = (lv_coord_t)(cy - 8);
        a.x2 = (lv_coord_t)(cx + 4);
        a.y2 = (lv_coord_t)(cy - 1);
        lv_draw_rect(draw_ctx, &rd, &a);
        menu_icon_line(draw_ctx, col, (lv_coord_t)(cx - 6), (lv_coord_t)(cy + 2),
                       (lv_coord_t)(cx + 6), (lv_coord_t)(cy + 8));
        break;
    case UI3_MENU_ICON_TRASH:
        menu_icon_line(draw_ctx, col, (lv_coord_t)(cx - 6), (lv_coord_t)(cy - 6),
                       (lv_coord_t)(cx + 6), (lv_coord_t)(cy - 6));
        menu_icon_line(draw_ctx, col, (lv_coord_t)(cx - 3), (lv_coord_t)(cy - 8),
                       (lv_coord_t)(cx - 3), (lv_coord_t)(cy - 6));
        menu_icon_line(draw_ctx, col, (lv_coord_t)(cx + 3), (lv_coord_t)(cy - 8),
                       (lv_coord_t)(cx + 3), (lv_coord_t)(cy - 6));
        menu_icon_line(draw_ctx, col, (lv_coord_t)(cx - 4), (lv_coord_t)(cy - 2),
                       (lv_coord_t)(cx - 3), (lv_coord_t)(cy + 7));
        menu_icon_line(draw_ctx, col, (lv_coord_t)(cx + 4), (lv_coord_t)(cy - 2),
                       (lv_coord_t)(cx + 3), (lv_coord_t)(cy + 7));
        menu_icon_line(draw_ctx, col, (lv_coord_t)(cx - 3), (lv_coord_t)(cy + 7),
                       (lv_coord_t)(cx + 3), (lv_coord_t)(cy + 7));
        break;
    case UI3_MENU_ICON_SEARCH:
        lv_draw_rect_dsc_init(&rd);
        rd.bg_opa = LV_OPA_TRANSP;
        rd.border_color = col;
        rd.border_width = 2;
        rd.radius = LV_RADIUS_CIRCLE;
        a.x1 = (lv_coord_t)(cx - 6);
        a.y1 = (lv_coord_t)(cy - 7);
        a.x2 = (lv_coord_t)(cx + 2);
        a.y2 = (lv_coord_t)(cy + 1);
        lv_draw_rect(draw_ctx, &rd, &a);
        menu_icon_line(draw_ctx, col, (lv_coord_t)(cx + 1), (lv_coord_t)(cy + 1),
                       (lv_coord_t)(cx + 7), (lv_coord_t)(cy + 7));
        break;
    case UI3_MENU_ICON_LIST:
        menu_icon_line(draw_ctx, col, (lv_coord_t)(cx - 7), (lv_coord_t)(cy - 5),
                       (lv_coord_t)(cx + 7), (lv_coord_t)(cy - 5));
        menu_icon_line(draw_ctx, col, (lv_coord_t)(cx - 7), (lv_coord_t)(cy),
                       (lv_coord_t)(cx + 7), (lv_coord_t)(cy));
        menu_icon_line(draw_ctx, col, (lv_coord_t)(cx - 7), (lv_coord_t)(cy + 5),
                       (lv_coord_t)(cx + 3), (lv_coord_t)(cy + 5));
        break;
    case UI3_MENU_ICON_WIFI:
        menu_icon_line(draw_ctx, col, (lv_coord_t)(cx - 8), (lv_coord_t)(cy + 2),
                       (lv_coord_t)(cx + 8), (lv_coord_t)(cy + 2));
        menu_icon_line(draw_ctx, col, (lv_coord_t)(cx - 5), (lv_coord_t)(cy - 2),
                       (lv_coord_t)(cx + 5), (lv_coord_t)(cy - 2));
        menu_icon_line(draw_ctx, col, (lv_coord_t)(cx - 2), (lv_coord_t)(cy - 6),
                       (lv_coord_t)(cx + 2), (lv_coord_t)(cy - 6));
        lv_draw_rect_dsc_init(&rd);
        rd.bg_color = col;
        rd.bg_opa = LV_OPA_COVER;
        rd.radius = LV_RADIUS_CIRCLE;
        a.x1 = (lv_coord_t)(cx - 2);
        a.y1 = (lv_coord_t)(cy + 4);
        a.x2 = (lv_coord_t)(cx + 2);
        a.y2 = (lv_coord_t)(cy + 8);
        lv_draw_rect(draw_ctx, &rd, &a);
        break;
    case UI3_MENU_ICON_LINK:
        menu_icon_line(draw_ctx, col, (lv_coord_t)(cx - 7), (lv_coord_t)(cy - 1),
                       (lv_coord_t)(cx - 2), (lv_coord_t)(cy - 1));
        menu_icon_line(draw_ctx, col, (lv_coord_t)(cx + 2), (lv_coord_t)(cy + 1),
                       (lv_coord_t)(cx + 7), (lv_coord_t)(cy + 1));
        menu_icon_line(draw_ctx, col, (lv_coord_t)(cx - 2), (lv_coord_t)(cy - 4),
                       (lv_coord_t)(cx + 2), (lv_coord_t)(cy));
        menu_icon_line(draw_ctx, col, (lv_coord_t)(cx - 2), (lv_coord_t)(cy + 2),
                       (lv_coord_t)(cx + 2), (lv_coord_t)(cy + 6));
        break;
    default:
        break;
...
lv_obj_t *ui3_list_card(lv_obj_t *parent, const char *title, const char *sub, bool sel)
{
    ...
    if(sel) {
        lv_obj_add_style(c, ui3_style_card_sel(), 0);
    }
                        ...
                        bool sel, lv_event_cb_t on_click, void *user)
{
    ...
    lv_obj_remove_style_all(c);
    lv_obj_set_width(c, lv_pct(100));
    lv_obj_set_height(c, UI3_MENU_CARD_H);
    lv_obj_add_style(c, ui3_style_card(), 0);
    if(sel) {
        lv_obj_add_style(c, ui3_style_card_sel(), 0);
    }
    ...
    lv_obj_set_size(ico, UI3_MENU_ICON_SZ, UI3_MENU_ICON_SZ);
    lv_obj_align(ico, LV_ALIGN_LEFT_MID, UI3_MENU_ICON_X, 0);
    ...
    lv_obj_set_user_data(ico, (void *)(uintptr_t)icon);
    lv_obj_add_event_cb(ico, menu_icon_draw_cb, LV_EVENT_DRAW_MAIN, NULL);

    t = lv_label_create(c);
    lv_label_set_text(t, title);
    lv_obj_set_style_text_font(t, UI3_FONT_HEAD, 0);
    lv_obj_set_style_text_color(t, UI3_COL_INK, 0);
    lv_obj_set_width(t, 150);
    lv_label_set_long_mode(t, LV_LABEL_LONG_DOT);
    lv_obj_align(t, LV_ALIGN_LEFT_MID, UI3_MENU_TEXT_X, 0);
    lv_obj_clear_flag(t, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_clear_flag(t, LV_OBJ_FLAG_CLICKABLE);

    chev = lv_label_create(c);
                    ...
                    lv_event_cb_t on_menu, lv_event_cb_t on_unlock, lv_event_cb_t on_lock)
{
    ...
    lv_obj_set_style_text_letter_space(st->lbl_date, 0, 0);
    lv_obj_set_style_pad_top(st->lbl_date, 0, 0);
    lv_obj_set_style_pad_bottom(st->lbl_date, 10, 0);
                          ...
                          lv_obj_t **out_val_lbl)
{
    ...
    lv_obj_set_width(col, show_chg ? 158 : 200);
...
lv_obj_t *ui3_user_row(lv_obj_t *parent, const char *acc, bool admin, bool sel)
{
    ...
    lv_obj_align(acc_l, LV_ALIGN_LEFT_MID, 8, 0);
                        ...
                        bool connected, bool sel, lv_event_cb_t cb, void *user)
{
    ...
    ico = lv_label_create(c);
    lv_label_set_text(ico, LV_SYMBOL_WIFI);
    lv_obj_set_style_text_font(ico, UI3_FONT_SYMBOL, 0);
    lv_obj_set_style_text_color(ico, (rssi >= 3u) ? UI3_COL_SAGE_MID : UI3_COL_INK_FAINT, 0);
    lv_obj_align(ico, LV_ALIGN_LEFT_MID, 6, 0);

    nm = lv_label_create(c);
    lv_label_set_text(nm, ssid);
    lv_obj_set_style_text_font(nm, UI3_FONT_WIFI, 0);
    lv_obj_add_style(nm, ui3_style_label(), 0);
    lv_obj_set_width(nm, connected ? 118 : 168);
    ...
    if(connected) {
        ...
        lv_obj_set_style_text_font(tg, UI3_FONT_WIFI_TAG, 0);
...
{
    lv_obj_t *zone = ui3_body_fill(lo, 56, 52);
    ...
    lv_obj_set_style_pad_row(zone, 10, 0);
    ...
    lv_label_set_text(cloud, (status != NULL) ? status : "");
    lv_obj_add_style(cloud, ui3_style_label(), 0);
    lv_obj_set_style_text_color(cloud, app_pair_cloud_ready() ? UI3_COL_SAGE : UI3_COL_INK_FAINT, 0);
    lv_obj_set_width(cloud, 210);
    lv_obj_set_style_text_align(cloud, LV_TEXT_ALIGN_CENTER, 0);
    lv_label_set_long_mode(cloud, LV_LABEL_LONG_WRAP);

    if(st->pair_code[0] != '\0' && cloud_aliyun_at_wifi_link_ready() != 0u && app_pair_is_bound() == 0u) {
                        ...
                        uint8_t fp_done, uint8_t nfc_done)
{
    ...
    lv_obj_set_height(bio, 112);
    lv_obj_add_style(bio, ui3_style_card(), 0);
    lv_obj_clear_flag(bio, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_obj_align(r1, LV_ALIGN_LEFT_MID, 0, -14);
    ...
    lv_label_set_text(a1, fp_done ? "已录入" : "录入");
    lv_obj_add_style(a1, ui3_style_label(), 0);
    lv_obj_set_style_text_color(a1, fp_done ? UI3_COL_SAGE : UI3_COL_TERRA, 0);
    ...
    lv_obj_align(r2, LV_ALIGN_LEFT_MID, 0, 12);
    ...
    lv_label_set_text(a2, nfc_done ? "已录入" : "录入");
    lv_obj_add_style(a2, ui3_style_label(), 0);
    lv_obj_set_style_text_color(a2, nfc_done ? UI3_COL_SAGE : UI3_COL_TERRA, 0);
    lv_obj_align(a2, LV_ALIGN_RIGHT_MID, -4, 0);

    lv_obj_t *hint = lv_label_create(bio);
    if(fp_done != 0u || nfc_done != 0u) {
        lv_label_set_text(hint, "一个NFC或指纹只能对应一个用户或管理员");
    } else {
        lv_label_set_text(hint, "每张凭证仅可绑定一名用户");
    }
    lv_obj_add_style(hint, ui3_style_label(), 0);
    lv_obj_set_style_text_font(hint, UI3_FONT_BODY, 0);
