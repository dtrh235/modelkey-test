...
void ui_nav_enter_screen_3(ui_nav_ctx_t *ctx, const ui_nav_ops_t *ops)
{
    ...
    from_scr = *ctx->app_scr;
    app_ui_nav_begin(from_scr);
...
void ui_nav_enter_screen_11(ui_nav_ctx_t *ctx)
{
    if(ctx == NULL || ctx->ui == NULL || ctx->app_scr == NULL) return;
    ui_load_scr_animation(ctx->ui, &ctx->ui->screen_11, ctx->ui->screen_11_del, &ctx->ui->screen_3_del,
                          setup_scr_screen_11, LV_SCR_LOAD_ANIM_NONE, 0, 0, false, true);
    *ctx->app_scr = APP_SCR_11;
}
...
void ui_nav_go_back_prev_screen(ui_nav_ctx_t *ctx, const ui_nav_ops_t *ops)
{
    ...
    } else if(*ctx->app_scr == APP_SCR_11) {
        WIFI_DBG("nav back scr11 -> scr3");
        screen_wifi_exit_cleanup();
        ui_load_scr_animation(ctx->ui, &ctx->ui->screen_3, ctx->ui->screen_3_del, &ctx->ui->screen_11_del,
                              setup_scr_screen_3, LV_SCR_LOAD_ANIM_NONE, 0, 0, false, true);
        *ctx->app_scr = APP_SCR_3;
        if(ctx->screen3_pending_index != NULL) *ctx->screen3_pending_index = 4u;
        if(ctx->screen3_need_init != NULL) *ctx->screen3_need_init = 1u;
    ...
    app_ui_nav_end(*ctx->app_scr);
    NAV_LOG("[NAV] back %u->%u ms=%lu\r\n", (unsigned)from_scr, (unsigned)*ctx->app_scr,
