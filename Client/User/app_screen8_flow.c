...
static uint32_t s_screen8_success_popup_deadline_ms = 0u;
...
static void screen8_show_result_popup(bool ok)
{
    ...
    if(ok) {
        ...
        s_screen8_success_popup_deadline_ms = HAL_GetTick() + 1500u;
    } else {
        s_screen8_success_popup_pending = 0u;
        s_screen8_success_popup_deadline_ms = 0u;
    }
}
...
{
    if(!s_screen8_success_popup_pending) return;
    if(g_app_scr != APP_SCR_8) {
        s_screen8_success_popup_pending = 0u;
        s_screen8_success_popup_deadline_ms = 0u;
        return;
    }
    if((int32_t)(HAL_GetTick() - s_screen8_success_popup_deadline_ms) < 0) return;
    s_screen8_success_popup_pending = 0u;
    s_screen8_success_popup_deadline_ms = 0u;
