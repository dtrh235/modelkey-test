...
static void app_touch_home(lv_coord_t x, lv_coord_t y)
{
    if(app_touch_hit(guider_ui.screen_btn_2, x, y)) {
        app_touch_two_tap(0u, g_lock_btn_selected ? 0u : 1u, lock_btn_set_selected, 0u);
        return;
    }
...
static void app_touch_screen8(lv_coord_t x, lv_coord_t y)
{
    ...
    if(g_screen8_popup != NULL && lv_obj_is_valid(g_screen8_popup)) {
        if(app_touch_hit(g_screen8_popup, x, y)) {
            app_touch_key_once(KEY_OK);
        }
        return;
    }
...
static void app_touch_screen9(lv_coord_t x, lv_coord_t y)
{
    if(g_screen9_msgbox_state == 1u) {
        if(app_touch_hit(g_screen9_popup_btn_yes, x, y)) {
            app_touch_key_once(KEY_LEFT);
            return;
        }
        if(app_touch_hit(g_screen9_popup_btn_no, x, y)) {
            app_touch_key_once(KEY_RIGHT);
            return;
        }
        if(g_screen9_popup != NULL && app_touch_hit(g_screen9_popup, x, y)) {
            uint8_t yes_sel = g_screen9_choice_yes ? 0u : 1u;
            app_touch_two_tap(10u, yes_sel, NULL, 0u);
            if(s_touch_slot == 10u) {
                app_key_ui_dispatch(KEY_OK);
                app_touch_reset_slot();
            }
...
static void app_touch_screen10(lv_coord_t x, lv_coord_t y)
{
    ...
    if(app_touch_hit(guider_ui.screen_10_btn_1, x, y)) {
        app_touch_two_tap(2u, g_screen10_focus, screen10_set_focus, 2u);
        return;
    }
    app_touch_reset_slot();
}
...
static void app_touch_screen7(lv_coord_t x, lv_coord_t y)
{
    if(g_screen7_msgbox_state == 1u) {
        if(app_touch_hit(g_screen7_popup_btn_yes, x, y)) {
            app_touch_key_once(KEY_LEFT);
            return;
        }
        if(app_touch_hit(g_screen7_popup_btn_no, x, y)) {
            app_touch_key_once(KEY_RIGHT);
            return;
        }
        if(g_screen7_popup != NULL && app_touch_hit(g_screen7_popup, x, y)) {
            uint8_t yes_sel = g_screen7_choice_yes ? 0u : 1u;
            app_touch_two_tap(10u, yes_sel, NULL, 0u);
            if(s_touch_slot == 10u) {
                app_key_ui_dispatch(KEY_OK);
                app_touch_reset_slot();
...
static void app_touch_process_release(lv_coord_t x, lv_coord_t y)
{
    ...
    if(g_app_scr == APP_SCR_5) {
        if(lv_scr_act() != guider_ui.screen_5) {
            return;
        }
    ...
    if(g_app_scr == APP_SCR_6) {
        if(lv_scr_act() != guider_ui.screen_6 && g_screen6_dlg == SCREEN6_DLG_NONE) {
            return;
        }
