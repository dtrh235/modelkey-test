...
static void as608_send_data_packet(const uint8_t *data, uint16_t data_len)
{
    uint16_t len_field = (uint16_t)(data_len + 2u);
    uint16_t sum = as608_pkg_checksum(0x02u, len_field, data, data_len);

    SendHead();
    SendAddr();
    SendFlag(0x02u);
    SendLength((int)len_field);
    {
        uint16_t i;
        for(i = 0u; i < data_len; i++) {
            Com_SendData(data[i]);
...
static void as608_send_end_packet(void)
{
    SendHead();
    SendAddr();
    SendFlag(0x07u);
...
uint8_t GZ_DownCharBuffer(uint8_t BufferID, const uint8_t *tpl_buf, uint16_t tpl_len)
{
    ...
    Com_SendData(BufferID);
    {
        uint16_t temp = (uint16_t)(0x01u + 0x04u + 0x09u + BufferID);
        SendCheck(temp);
    }
    data = JudgeStr(2000);
    if(data == NULL || data[9] != 0x00u) {
    ...
    for(off = 0u; off < AS608_TEMPLATE_SIZE; off += AS608_DATA_PKT_SZ) {
        chunk = AS608_DATA_PKT_SZ;
        as608_send_data_packet(tpl_buf + off, chunk);
        HAL_Delay(2);
    }
    as608_send_end_packet();
    if(as608_wait_rx_grow(12u, 400u) == 0u) {
        return 0xFFu;
    }
