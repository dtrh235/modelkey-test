...
void screen8_show_fp_enroll_popup(void)
{
    ...
    HAL_Delay(20);
    if(app_fp_hw_try_handshake(&g_fp_hw_inited) == 0u) {
    ...
    /* valid_n 是模块内模板个数，不是空闲页号；页号必须由用户表分配，避免覆盖管理员 57/58。 */
    ...
    if(g_fp_enroll_step == 2u) {
        ...
        if(en == 0x00u) {
            ...
            if(g_app_scr == APP_SCR_10) {
                ...
                ENROLL_LOG("fp bind via screen10 acc=%s p1=%u p2=%u", g_screen5_found_acc,
                           (unsigned)g_fp_enroll_page_id, (unsigned)g_fp_enroll_page_id_2);
                if(g_screen5_found_acc[0] != '\0') {
                    ...
                    users_clear_fp_by_acc(g_screen5_found_acc, 0u);
                    /* 两次 bind 不要各触发一遍 RS485 镜像（否则 schedule/发包看起来像“发了两次”） */
                    ok1 = users_bind_fp_by_acc_ex(g_screen5_found_acc, g_fp_enroll_page_id, 0u) ? 1u : 0u;
                    ok2 = users_bind_fp_by_acc_ex(g_screen5_found_acc, g_fp_enroll_page_id_2, 0u) ? 1u : 0u;
                }
                if(ok1 != 0u || ok2 != 0u) {
#if (APP_RS485_ENABLE == 1) && APP_RS485_IS_MASTER
                    users_mirror_schedule_acc(g_screen5_found_acc);
                ...
                ENROLL_LOG("fp pending via screen8 acc(text)=%s", lv_textarea_get_text(guider_ui.screen_8_ta_1));
